...
impl<B: AutodiffBackend, M: AutodiffModule<B> + NoisePredictor<B>> DiffusionModel<B, M>
    where M: AutodiffModule<B>, {   
        ...
        let optim = AdamConfig::new().with_epsilon(1e-3).init();
    ...
    pub fn train(&mut self, train_data: Vec<Tensor<B, 3>>, val_data: Vec<Tensor<B, 3>>, epochs: usize) {
        ...
        for epoch in 0..epochs {
            ...
            println!(
                "epoch: {}/{}, (train, val) loss: {:.7}, {:.7} in {:.2?} seconds",
                epoch, epochs, epoch_train_loss, epoch_val_loss, t_eplased
            );
            ...
            for x_0 in train_data.iter() {
                    // let first = x_0.clone().slice([0..1, 0..self.res.0, 0..self.res.1]).squeeze::<2>();
                    // let vals: Vec<f32> = first.to_data().to_vec::<f32>().unwrap();
                    // for row in vals.chunks(self.res.1) {
                ...
                let (x_t, t_tensor, eps) = self.noise_data(x_0.clone());
                    ...
                    // let first = x_t.clone().slice([0..1, 0..self.res.0, 0..self.res.1]).squeeze::<2>();
                    // let vals: Vec<f32> = first.to_data().to_vec::<f32>().unwrap();
                    // for row in vals.chunks(self.res.1) {
                    //     let line: Vec<String> = row.iter().map(|v| format!("{:.2}", v)).collect();
                    //     println!("{}", line.join(" "));
                ...
                // let loss = (eps.clone() * eps_hat.clone().log() + (eps.ones_like() - eps.clone()) * (eps_hat.ones_like() - eps_hat.clone()).log()).neg().mean();

                batch_losses.push(loss.clone().into_scalar().elem());

                let grads = loss.backward();
                let grads = GradientsParams::from_grads(grads, &self.mlp);
                self.mlp = self.optim.step(1e-3, self.mlp.clone(), grads);
            }
    ...
    pub fn inference(&self) -> Tensor<B, 2> {
        let x = Tensor::<B, 3>::random([1, self.res.0,self.res.1],Distribution::Normal(0.0, 1.0),&self.device);
                    ...
                    let first = x.clone().slice([0..1, 0..self.res.0, 0..self.res.1]).squeeze::<2>();
                    let vals: Vec<f32> = first.to_data().to_vec::<f32>().unwrap();
                    println!("");
                    for row in vals.chunks(self.res.1) {
                        let line: Vec<String> = row.iter().map(|v| format!("{:.2}", v)).collect();
        ...
        let sample = sample.squeeze::<2>();
                    ...
                    let first = sample.clone().slice([0..self.res.0, 0..self.res.1]).squeeze::<2>();
                    let vals: Vec<f32> = first.to_data().to_vec::<f32>().unwrap();
                    println!("");
                    for row in vals.chunks(self.res.1) {
                        let line: Vec<String> = row.iter().map(|v| format!("{:.2}", v)).collect();
...
impl<B: Backend> ErrorCnn<B> {
    pub fn new(device: &B::Device, res: (usize, usize), t: usize) -> Self {
        Self {
            ...
            relu2: Relu::new(),
            conv3: Conv2dConfig::new([16, 1], [3, 3]).with_padding(burn::nn::PaddingConfig2d::Same).init(device),
        }
    }
